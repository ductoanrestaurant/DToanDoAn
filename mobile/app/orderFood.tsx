...
// --- INTERFACES ---
export interface DanhMuc { maDanhMuc: number; tenDanhMuc: string; }
...
type OrderFoodParams = {
    ...
    maKhachHang?: string; // Only for staff orders
};
...
const MenuScreen = () => {
    ...
    const { tableId, tableName, bookingTime, soLuongNguoi, updatedItems, verifyUser } = params;

    const [userInfo, setUserInfo] = useState<{ maKhachHang: string | null; maNhanVien: string | null }>({ maKhachHang: null, maNhanVien: null });
    ...
    useEffect(() => {
        const loadUserData = async () => {
            try {
                ...
                let nvId: string | null = null;

                if (verifyUser === 'nhanvien') {
                    ...
                    khId = customerInfo.maKhachHang;
                } else {
                    throw new Error("Loại người dùng không xác định.");
                }
                setUserInfo({ maKhachHang: khId, maNhanVien: nvId });
            } catch (err: any) {
                Alert.alert("Lỗi xác thực", err.message, [{ text: 'OK', onPress: () => router.replace('/login') }]);
            }
        };
        loadUserData();
    }, [verifyUser, params.maKhachHang]);
    ...
    const fetchData = useCallback(async (isRefreshing = false) => {
        ...
        try {
            ...
            const menuWithReviews = allProducts.map(p => ({ ...p, danhSachDanhGia: allReviews.filter(r => r.id.maSanPham === p.maSanPham) }));
    ...
    const handleViewCart = () => {
        if (!userInfo.maKhachHang) {
            Alert.alert("Lỗi", "Không thể xác định khách hàng. Vui lòng thử lại.");
            return;
        }
        const inforItemPush = mon.map(item => ({ maSanPham: item.maSanPham, soluong: item.soluong }));
        router.push({
            pathname: '/CartScreen',
            params: {
                tableId, tableName, bookingTime, soLuongNguoi,
    ...
    if (loading) return <View style={styles.centered}><ActivityIndicator size="large" color="#FF6600" /></View>;
    if (error) return <View style={styles.centered}><Text style={styles.emptyText}>{error}</Text></View>;
