...
// 1. IMPORT api VÀ ENDPOINTS TỪ FILE CẤU HÌNH CỦA BẠN
// (Lưu ý: Kiểm tra lại đường dẫn '../api/api' cho đúng với cấu trúc thư mục của bạn)
...
const ChatbotScreen = () => {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const response = await api.post(ENDPOINTS.AI_CHAT, {
                // Body này phải khớp với class DTO hoặc tham số bên Controller Spring Boot
                // Nếu Controller nhận String raw body thì gửi messageToSend thẳng
                // Nếu Controller nhận @RequestBody Map<String, String> payload thì gửi object:
                message: messageToSend
            });

            // 3. XỬ LÝ DỮ LIỆU TRẢ VỀ
            // Axios trả dữ liệu trong response.data
            // Nếu Spring Boot trả về String trực tiếp: botResponseText = response.data
            // Nếu Spring Boot trả về JSON { "reply": "..." }: botResponseText = response.data.reply

            // Code an toàn: Kiểm tra xem data là object hay string
            let botResponseText = '';
            if (typeof response.data === 'string') {
                ...
                botResponseText = JSON.stringify(response.data); // Fallback
            }
    ...
    // ... (Các phần Render và Styles giữ nguyên như cũ) ...

    useEffect(() => {
        if (flatListRef.current) {
            setTimeout(() => {
                flatListRef.current?.scrollToEnd({ animated: true });
            }, 100); // Thêm delay nhỏ để đảm bảo layout render xong mới scroll
        }
    }, [messages]);
...
// ... Styles giữ nguyên (Copy từ code cũ của bạn) ...
const styles = StyleSheet.create({
    ...
    userBubble: { backgroundColor: '#FF6600', borderBottomRightRadius: 4 }, // Màu cam thương hiệu quán ăn
